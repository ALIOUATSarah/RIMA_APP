      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-2 scrollbar-hide">
        {channel.messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center opacity-30 text-center px-12">
            <h3 className="text-xl font-bold mb-2">#{channel.title.toLowerCase()}</h3>
            <p className="text-sm font-light">Workspace conversation channel initialized.</p>
          </div>
        )}
                      ...
                      {showAvatar && !isSelf && (
                         <div className="flex items-center gap-2 mb-1 px-1">
                            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-600">
                                {isRima ? 'Rima Assistant' : (msg.sender as User).name}
                            </span>
                         </div>
                      )}
